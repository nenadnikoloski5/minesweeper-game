...
function init() {
  ...
  for (let row = 0; row < rows; row++)
    for (let column = 0; column < columns; column++) {
      ...
      tile[row][column].src = './icons/hidden.png';
...
function click(event) {
  ...
  if (event.which == 3) {
    switch (picture[row][column]) {
      case 'hidden':
        tile[row][column].src = './icons/flag.png';
        remaining--;
        picture[row][column] = 'flag';
        break;
      case 'flag':
        tile[row][column].src = './icons/question.png';
        remaining++;
  ...
  if (event.which == 1 && picture[row][column] != 'flag') {
    if (board[row][column] == 'mine') {
      for (let row = 0; row < rows; row++)
        for (let column = 0; column < columns; column++) {
          if (board[row][column] == 'mine') {
            tile[row][column].src = './icons/mine.png';
          }
          if (board[row][column] != 'mine' && picture[row][column] == 'flag') {
            tile[row][column].src = './icons/misplaced.png';
          }
        }
...
function reveal(row, column) {
  tile[row][column].src = "./icons/" + board[row][column] + '.png';
